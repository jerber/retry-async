...
from retry_async.api import retry_call_sync
...
def test_retry(monkeypatch):
    ...
    @retry(is_async=False, tries=tries, delay=delay, backoff=backoff)
    def f():
        hit[0] += 1
        1 / 0
...
def test_tries_inf():
    ...
    @retry(is_async=False, tries=float("inf"))
    def f():
        hit[0] += 1
        if hit[0] == target:
            return target
...
def test_max_delay(monkeypatch):
    ...
    @retry(
        is_async=False, tries=tries, delay=delay, max_delay=max_delay, backoff=backoff
    )
    def f():
        hit[0] += 1
        1 / 0
...
def test_fixed_jitter(monkeypatch):
    ...
    @retry(is_async=False, tries=tries, jitter=jitter)
    def f():
        hit[0] += 1
        1 / 0
...
def test_retry_call():
    ...
    tries = 2
    try:
        retry_call_sync(f_mock, exceptions=RuntimeError, tries=tries)
    except RuntimeError:
        pass
...
def test_retry_call_2():
    ...
    result = None
    try:
        result = retry_call_sync(f_mock, exceptions=RuntimeError, tries=tries)
    except RuntimeError:
        pass
...
def test_retry_call_with_args():
    ...
    try:
        result = retry_call_sync(f_mock, fargs=[return_value])
    except RuntimeError:
        pass
...
def test_retry_call_with_kwargs():
    ...
    try:
        result = retry_call_sync(f_mock, fkwargs=kwargs)
    except RuntimeError:
        pass
